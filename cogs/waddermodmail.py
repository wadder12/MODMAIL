        ...
        if not message.content.startswith('[]'):
            if isinstance(message.channel, nextcord.channel.DMChannel) and message.author != self.bot.user:
                ...
                try:
                    ...
                    embed_to.add_field(name='Sent by', value=message.author.mention, inline=True)
                    embed_to.add_field(name="Sender's Name", value=f"**{message.author}**", inline=True)
                    embed_to.add_field(name='User ID', value=f"`{message.author.id}`", inline=True)
                    ...
                    embed_reply = nextcord.Embed(
                        title='✅ Mail sent',
                        description="Your message has been sent to moderators",
                        color=nextcord.Color.green(),
                        timestamp=message.created_at
                    )
                    ...
                    embed_reply = nextcord.Embed(
                        title='❌ Sorry 😔',
                        description="I couldn't send your message to moderators",
                        color=nextcord.Color.red(),
                        timestamp=message.created_at
                    )
                    await message.author.send(embed=embed_reply)
